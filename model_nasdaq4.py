...
#
# Model 4: Try to get model to ignore most stocks. Try separate channels for longs
# and shorts, with ReLU on each.
#
#
...
class fc_layers(nn.Module):
    def __init__(self, n_symbols, n_features, output_sizes, dropout=0):
        ...
        self.fc3a = nn.Linear(output_sizes[1], output_sizes[2], bias=True)
        self.fc3b = nn.Linear(output_sizes[1], output_sizes[2], bias=True)
    ...
    def forward(self, input):
        ...
        # TODO: How to link long and short positions more directly?
        outlong = self.fc3a(output)
        outlong = F.relu(outlong)

        outshort = self.fc3b(output)
        outshort = -F.relu(outshort)

        output = outlong + outshort
...
for i_iter in range(n_iters):
    ...
    loss_num_trades = torch.mean(torch.abs(output))  # want most positions to be zero
    lambda_num_trades = 1.0
    loss = loss + lambda_num_trades*loss_num_trades
